...
- spec.json メタデータを更新:
  ...
  - `ready_for_implementation: false` を維持または設定（タスク承認前は実装不可）
...
5. **状態更新**: 承認待ちのフェーズと、再実行が必要なゲートを明記
...
- **提案アクション**: "`/tasks $1 -y` を実行して両方を自動承認して進行"
...
**実装開始前**:
- **重要**: `/impl` 実行前に会話履歴をクリアしてコンテキストを解放
- これは最初のタスク開始時またはタスク切り替え時に適用
- 新鮮なコンテキストでクリーンな状態と適切なタスクフォーカスを確保

**タスク承認後**:
- 特定のタスクを実行: `/impl $1 1.1`（推奨: 各タスク間でコンテキストをクリア）
- 複数タスクを実行: `/impl $1 1.1,1.2`（注意して使用、タスク間でコンテキストをクリア）
- 引数なし: `/impl $1`（すべての保留タスクを実行 - コンテキスト膨張のため非推奨）

**修正が必要な場合**:
- フィードバックを提供し `/tasks $1` を再実行
