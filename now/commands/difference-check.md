...
4. **状態影響**: review / final-check / trace の再実行が必要かを明記
...
### エラーシナリオ
- **要件が見つからない**: requirements.md が存在しない場合、メッセージで停止: "先に `/requirements $1` を実行して要件を生成してください"
...
**ギャップ分析完了時**:
- ギャップ分析の洞察をレビュー
- `/design $1` を実行して技術設計ドキュメントを作成
- または `/design $1 -y` で要件を自動承認し直接進行
